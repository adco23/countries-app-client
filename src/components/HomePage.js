...
import { alphabOrder, filterActivity, filterContinent, getActivities, getAllCountries, populationOrder } from "../redux/action";
...
const HomePage = () => {
  ...
  const lastPage = Math.ceil((countries.length - 9)/ (countriesPerPage + 1)) + 1;
  ...
  const currentCountries = countries.slice(indexFisrtCountry + FirstCountry, indexLastCountry + LastCountry);
  ...
  return (
    <div className={style.HomePage}>
      ...
      <div className={style.pagination}>
        <p><strong>Page: </strong>{`${currentPage} / ${lastPage}`}</p>
        <p><strong>Countries: </strong>{countries.length}</p>
        <Pagination 
